  ...
  return buildCredential({
    ...
    namespaces: {
      ...
      'org.iso.23220.education.transcript.1': [
        ...
        ['institution_id', new Cbor().tstr('smartacademy.example').encode()],
        ['institution_id_scheme', new Cbor().tstr('schac').encode()],
        ['institution_ror', new Cbor().tstr('https://ror.org/04demo123').encode()],
        ['institution_name_alt', new Cbor().tstr('Smart Academie').encode()],
        ['language_of_instruction', new Cbor().tstr('en').encode()],
        ['student_id', new Cbor().tstr('SA-TRUST-1').encode()],
        ['student_id_scheme', new Cbor().tstr('institution-student-number').encode()],
        ['programme_title', new Cbor().tstr('Bachelor of Computer Science').encode()],
        ['programme_title_alt', new Cbor().tstr('Informatica').encode()],
      ...
      'org.iso.23220.education.academic-record.1': [
        ['document_type', new Cbor().tstr('academic-record').encode()],
        ['document_status', new Cbor().tstr('official').encode()],
        ['document_completeness', new Cbor().tstr('partial').encode()],
        ['transcript_type', new Cbor().tstr('official-transcript').encode()],
        ['attesting_office', new Cbor().tstr('Office of the Registrar').encode()],
        ['attesting_capacity', new Cbor().tstr('Registrar').encode()],
      ],
    },
  });
}
