...
private fun CertificateCard(summary: CredentialSummary?, onClick: () -> Unit, modifier: Modifier = Modifier) {
    ...
    val institution = summary?.institution?.takeIf { it.isNotBlank() } ?: "Smart College"
    ...
    Card(
        ...
        Column(Modifier.padding(20.dp)) {
            Text("Academic Certificate", style = MaterialTheme.typography.labelLarge)
            Spacer(Modifier.height(12.dp))
            Text(personName, style = MaterialTheme.typography.headlineSmall)
            Spacer(Modifier.height(8.dp))
            Text(institution, style = MaterialTheme.typography.titleMedium)
            Spacer(Modifier.height(16.dp))
            Text(degreeLevel, style = MaterialTheme.typography.bodyLarge)
            Text("Graduation: $graduationDate", style = MaterialTheme.typography.bodyMedium)
            Spacer(Modifier.height(12.dp))
            Text("Tap to view credential details", style = MaterialTheme.typography.labelMedium)
        }
    }
}
